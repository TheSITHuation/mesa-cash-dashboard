...
// Asegúrate de que tu archivo /src/services/auth.js exporte estas funciones.
import {
  onAuthStateChanged,
  getCurrentRole,
  login
} from '/src/services/auth.js';
import { ROLE_STAFF } from '/src/services/auth.js';

// Tu configuración de Firebase. Es mejor tenerla en un solo lugar.
const firebaseConfig = {
    apiKey: "AIzaSyAtX_lsgN49FPvXwyOslKdFOIjwJDzqqKk", // Considera usar variables de entorno para seguridad
...
// =================================================================
// 2. REFERENCIAS AL DOM Y FUNCIONES DE UI
// =================================================================
...
// Funciones claras para manejar la visibilidad
...
// =================================================================
// 3. NÚCLEO DE AUTENTICACIÓN
// =================================================================

// Un único y centralizado observador de autenticación.
onAuthStateChanged(async (user) => {
  if (user) {
    const role = await getCurrentRole(user);
    if (role === 'staff') {
      console.log("Acceso concedido. Rol: Staff.");
      showDashboardUI();
      initStaffApp(); // Arrancamos la aplicación principal de staff.
    } else {
      console.warn("Acceso denegado. Rol no es 'staff'. Redirigiendo...");
      await logout();
      
    }
  } else {
    console.log("Ningún usuario logueado. Mostrando formulario de acceso.");
    showLoginUI();
  }
});
...
loginBtn.addEventListener('click', async () => {
  ...
  if (!email || !pass) {
      Swal.fire('Campos vacíos', 'Por favor, introduce email y contraseña.', 'warning');
      return;
  }
  try {
    ...
    Swal.fire('Error de Acceso', 'Credenciales incorrectas. Por favor, inténtalo de nuevo.', 'error');
  }
});

// =================================================================
// 4. LÓGICA PRINCIPAL DE LA APLICACIÓN DE STAFF
// =================================================================
// Esta función SÓLO se llama cuando el usuario ha sido verificado como 'staff'.
// Una bandera para asegurar que la app no se inicialice múltiples veces.
let isAppInitialized = false;

function initStaffApp() {
    if (isAppInitialized) return; // Si ya está inicializada, no hacemos nada.
    isAppInitialized = true;

    console.log("App.js: Iniciando aplicación de Staff...");
    ...
    async function startSession(tableId) {
        const record = {
            tableId, creationTime: serverTimestamp(), players: [], buyIns: [], mainTimerSeconds: 0,
            mainTimerStartTime: null, marqueeText: "✨ Skampa Poker Room Anuncios! ✨",
            tableVisualNumber: String(document.querySelectorAll('.table-section').length + 1),
            gameType: 'NLHE', blinds: '25 / 25', buyinRange: '$1,000 - $5,000',
            tableStatus: STATUS_INACTIVE, dealerImageIndex: 0, playerCount: 0, waitlistCount: 0, waitlistPlayers: []
        };
        for (let i = 1; i <= 9; i++) {
            record[`seat_${i}_player`] = null; record[`seat_${i}_avatar`] = null; record[`seat_${i}_seconds`] = 0; record[`seat_${i}_startTime`] = null;
        }
    ...
    async function occupySeat(playerName, avatarName, tableId, seatNumber) {
        ...
        const updateData = {
            [`seat_${seatNumber}_player`]: playerName, [`seat_${seatNumber}_avatar`]: avatarDisplayValue,
    ...
    async function vacateSeat(playerName, seatNumber, tableId) {
        const record = await getRecord(tableId); if (!record) return;
        const updateData = {
    ...
    // --- EJECUCIÓN INICIAL DENTRO DE initStaffApp ---
    if (addTableButton) {
        addTableButton.addEventListener('click', () => startSession(generateTableId()));
    }

    // Recomiendo añadir un botón de logout con id="logoutBtn" en tu HTML.
